...
func (test *ServiceProviderTest) TestCanHandleSignedAssertionsResponse(c *C) {
	// An actual response from onelogin
	TimeNow = func() time.Time {
		rv, _ := time.Parse("Mon Jan 2 15:04:05 UTC 2006", "Tue Jul 17 01:02:20 UTC 2014")
		return rv
	}
...
`
	s := ServiceProvider{
		Key:         test.Key,
		Certificate: test.Certificate,
		MetadataURL: "http://sp.example.com/demo1/metadata.php",
	...
	c.Assert(assertion.Subject.NameID.Value, DeepEquals, "_ce3d2948b4cf20146dee0a0b3dd6f69b6cf86f62d7")
	c.Assert(assertion.AttributeStatement.Attributes, DeepEquals, []Attribute{
		{
			Name:       "uid",
			NameFormat: "urn:oasis:names:tc:SAML:2.0:attrname-format:basic",
