...
func compareStrings(actual, expected string, c *C) {
	...
	if divergedAt >= 0 {
		startIndex := divergedAt - 20
		if startIndex < 0 {
			startIndex = 0
		}
		endIndex := divergedAt + 20
		if endIndex > min {
			endIndex = min
		}
		c.Errorf("Strings diverged at index %d: \n%s\n%s", divergedAt, actual[startIndex:endIndex], expected[startIndex:endIndex])
	}
}
